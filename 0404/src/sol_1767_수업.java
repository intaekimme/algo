...
public class sol_1767_수업 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for(int tc = 1; tc<=T; tc++) {
			...
			max = 0; // 최대 연결 코어수 
			min = Integer.MAX_VALUE; // 최소 길이의 합
	...
	private static void go(int index, int cCnt) { // 부붅집합으로 코어 전선놓기 시도, cCnt : 전원과 연결된 코어수
		...
		for(int d=0; d<4; d++) {
			if(isAvailable(r, c, d)) { // 현재 코어의 r, c 위치에서 d 방향으로 전선을 놓을 수 있다면
				...
				go(index+1, cCnt+1);	// 다음 코어로 가기
				setStatus(r, c, d, 0); // 전선 원복(지우기)
			}
		}
		// 전선 놓지 않기
		go(index + 1, cCnt);
